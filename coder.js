...
var filesRef = database.ref('files'); // Perbaiki referensi jika perlu
...
window.onload = function() {
    ...
    filesRef.orderByChild('views').on('value', function(snapshot) {
        ...
        snapshot.forEach(function(childSnapshot) {
            ...
            var userProfile = fileData.userProfile || {}; // Ambil data profil pengguna
            ...
            var lastModifiedPromise = getLastModified(fileName).then(function(lastModified) {
                var formattedLastModified = lastModified ? lastModified.toLocaleDateString(undefined, {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                    hour: '2-digit',
                ...
                filesData.push({
                    ...
                    userProfile: userProfile // Simpan data profil pengguna
                });
            });
        ...
        Promise.all(getLastModifiedPromises).then(function() {
            ...
            filesData.forEach(function(file) {
                addFileToTable(file.name, file.url, file.size, file.views, file.lastModified, file.userProfile);
            });
...
function displayFilesInTable(filesData) {
    ...
    var headers = ['Profile', 'Name', 'Size', 'Link', 'Views', 'Created'];
...
function addFileToTable(fileName, fileURL, fileSize, views, lastModified, userProfile) {
    var table = document.getElementById('file-table');
    var newRow = table.insertRow();
    var cell1 = newRow.insertCell(0); // Profile
    var cell2 = newRow.insertCell(1); // Name
    var cell3 = newRow.insertCell(2); // Size
    var cell4 = newRow.insertCell(3); // Link
    var cell5 = newRow.insertCell(4); // Views
    var cell6 = newRow.insertCell(5); // Created

    // Tampilkan informasi profil pengguna
    cell1.innerHTML = '<img src="' + (userProfile.photoURL || '') + '" alt="Profile Photo" style="width: 50px; height: 50px; border-radius: 50%;">' +
                      '<p>' + (userProfile.displayName || 'No Display Name') + '</p>';

    cell2.innerHTML = '<a href="#" class="file-link">' + truncateFileName(fileName, 20) + '</a>';
    cell3.innerHTML = fileSize;
...
function updateUI(user) {
    if (user) {
        ...
        document.getElementById('userNameEmail').textContent = user.displayName || 'No Display Name';
    } else {
...
function uploadFile(file) {
    ...
    if (user) {
        ...
        var fileRef = storageRef.child('html-files/' + file.name);
        ...
        uploadTask.on('state_changed', function(snapshot) {
            ...
            uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                var fileData = {
                    ...
                    size: file.size,
                    views: 0,
                    lastModified: new Date().toISOString(), // Simpan waktu upload sebagai lastModified
                    userProfile: userProfile
                };

                filesRef.push(fileData).then(function() {
                    console.log('File and user profile data saved successfully.');
                }).catch(function(error) {
                    console.error('Error saving file and user profile data:', error);
                });
            });
        });
    } else {
